...
bool is_Magnetometer_present = false; // Only for simulator
bool HOME = false;
      ...
      y_last = 0.0;
        ...
        if(sim) //Mohsen
        {
          ...
          hat_P_ENU.y = -DY_sim;
				...
				outputEstimatedPosition_.altitude  = 0;

				pubEstimatedPosition_.publish(outputEstimatedPosition_);
			}
		}

    void sim_Handle(tf::TransformListener* listener)
		{
			...
			Sim_Yaw = tf::getYaw(transform.getRotation());
      DX_sim = transform.getOrigin().getX() - x_last;
      DY_sim = transform.getOrigin().getY() - y_last;
      x_last = transform.getOrigin().getX();
      y_last = transform.getOrigin().getY();
		}
      ...
      {
        if(sim) ROS_WARN("Simulation mode!");
        is_Magnetometer_present = true;
      } else sim = false;
